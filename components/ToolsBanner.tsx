...
const TOOLS: Tool[] = [
  ...
  { name: 'GoHighLevel',      initials: 'GHL', color: '#F97316', desc: 'All-in-one CRM and marketing automation for agencies. Includes funnels, email, SMS, and pipeline management.' },
  ...
  { name: 'Zoho',             si: siZoho,         desc: 'Suite of business apps including CRM, HR, finance, and project management — all integrated in one ecosystem.' },
  ...
  { name: 'Looker Studio',    si: siLooker,       desc: "Google's free data visualization tool. Builds dashboards connecting to Sheets, BigQuery, and other data sources." },
]
  ...
  return (
    <div style={{ perspective: 600 }} onMouseMove={onMove} onMouseLeave={onLeave}>
      <motion.div
        ...
        className="flex flex-col items-center gap-3 px-4 py-4 rounded-xl border border-white/10 bg-zinc-900/60 flex-shrink-0 w-[144px] cursor-pointer hover:border-white/22 transition-colors"
        style={{ rotateX, rotateY, transformStyle: 'preserve-3d' }}
        whileTap={{ scale: 0.95 }}
      >
        {isIcon ? (
          <svg viewBox="0 0 24 24" className="w-8 h-8 flex-shrink-0" style={{ fill: iconFill! }} aria-hidden>
            <path d={tool.si.path} />
          </svg>
        ) : (
          <div
            className="w-8 h-8 rounded-lg flex items-center justify-center text-white font-black flex-shrink-0 border border-white/25"
            style={{ backgroundColor: tool.color, fontSize: (tool as FallbackTool).initials.length > 2 ? 9 : 12 }}
          >
...
export default function ToolsBanner() {
  ...
  const selectedFill = isSelectedIcon
    ? (selected as IconTool).si.hex === '000000' ? '#d4d4d8' : `#${(selected as IconTool).si.hex}`
    : null
